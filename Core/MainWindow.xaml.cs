...
public sealed partial class MainWindow : Window
{
    ...
    private void TitleBar_PointerPressed(object sender, PointerRoutedEventArgs e) { if (e.GetCurrentPoint((UIElement)sender).Properties.IsLeftButtonPressed) InputNonClientPointerSource.GetForWindowId(this.AppWindow.Id).StartDraggingPointerDrag(); }
    ...
    private void Omnibox_QuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
    {
        ...
        if (!url.StartsWith("http://") && !url.StartsWith("https://")) url = url.Contains(".") ? $"https://{url}" : $"https://www.google.com/search?q={Uri.EscapeDataString(url)}";
