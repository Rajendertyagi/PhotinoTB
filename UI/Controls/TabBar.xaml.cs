...
namespace TB_Browser.UI.Controls
{
    public partial class TabBar : UserControl
    {
        ...
        public TabBar(ITabService svc) { InitializeComponent(); _svc = svc; _svc.TabAdded += (_, t) => AddTabUI(t); _svc.TabRemoved += (_, t) => RemoveTabUI(t.Id); }

        private void AddTabUI(TabModel t)
        {
            var grid = new StackPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(0,0,4,0) };
            var btn = new Button { Content = t.Title, Style = (Style)FindResource("TabBtn"), MinWidth = 120, MaxWidth = 200, Tag = t.Id };
            var close = new Button { Content = "✕", Width = 20, Height = 20, Background = Brushes.Transparent, BorderThickness = new Thickness(0), Foreground = Brushes.White, Margin = new Thickness(4,0,0,0) };
        ...
        private void RemoveTabUI(int id) { foreach (StackPanel sp in TabsPanel.Children) if ((int)((Button)sp.Children[0]).Tag == id) { TabsPanel.Children.Remove(sp); break; } }
